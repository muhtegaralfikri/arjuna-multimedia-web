        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden flex flex-col transition-all hover:shadow-md hover:border-indigo-200">
            <div class="p-6 flex-grow border-b border-slate-100">
                <div class="flex items-center justify-between mb-3">
                    <h3 class="text-lg font-bold text-slate-900">{{ $page->title }}</h3>
                ...
                @if($page->hero_title)
                    <div class="mb-4">
                        ...
                        <p class="text-sm text-slate-700 italic">"{{ \Illuminate\Support\Str::limit($page->hero_title, 60) }}"</p>
                    </div>
                @endif
                ...
                @if($page->meta_title)
                    <div>
                        ...
                        <p class="text-sm text-indigo-700">{{ \Illuminate\Support\Str::limit($page->meta_title, 50) }}</p>
                    </div>
                @endif
            </div>
            
            <div class="p-4 bg-slate-50 relative group">
                <a href="{{ route('admin.pages.edit', $page->slug) }}" class="flex items-center justify-center w-full py-2 bg-white border border-slate-300 rounded-xl font-medium text-sm text-slate-700 group-hover:bg-indigo-600 group-hover:text-white group-hover:border-indigo-600 transition-all shadow-sm">
