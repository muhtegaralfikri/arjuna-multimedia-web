...
<div class="mb-6 flex items-center justify-between">
    <div>
        ...
        <p class="text-slate-500 text-sm mt-1">Lakukan perubahan pada data area <strong class="text-slate-700">{{ $area->name }}</strong>.</p>
    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        <select id="status" name="status" required
                            ...
                            <option value="available" {{ $area->status === 'available' ? 'selected' : '' }}>Bisa Dipasang (Tersedia)</option>
                            <option value="coming_soon" {{ $area->status === 'coming_soon' ? 'selected' : '' }}>Pembangunan Jaringan (Segera Hadir)</option>
                            <option value="paused" {{ $area->status === 'paused' ? 'selected' : '' }}>Jaringan Penuh / Dibatasi (Jeda)</option>
                        </select>
                ...
                <div class="col-span-1 md:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="3"
                        ...
                        placeholder="Deskripsi wilayah layanan atau pesan khusus untuk calon pelanggan di area ini...">{{ old('description', $area->description) }}</textarea>
                    @error('description')
                        <p class="mt-1.5 text-sm text-rose-600 flex items-center"><svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="col-span-1 md:col-span-2">
                    ...
                    <textarea id="coverage_detail" name="coverage_detail" rows="3"
                        class="w-full px-4 py-3 bg-slate-50 border border-slate-200 text-slate-900 rounded-xl focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all shadow-sm resize-y"
                        placeholder="Misalnya: RT 01-05 Desa A, Gang Flamboyan, Perumahan B Blok C...">{{ old('coverage_detail', $area->coverage_detail) }}</textarea>
                ...
                <div class="col-span-1 md:col-span-2">
                    <label class="relative flex items-center group cursor-pointer w-max">
                        <input type="checkbox" name="is_active" value="1" {{ $area->is_active ? 'checked' : '' }} class="peer sr-only">
