...
<div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
    ...
    <a href="{{ route('admin.areas.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-xl font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-all shadow-sm">
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden">
                        ...
                        <td class="py-4 px-6 whitespace-nowrap">
                            <div class="flex items-center">
                                ...
                                <div class="flex flex-col">
                                    ...
                                    <span class="md:hidden block text-xs text-slate-500 mt-0.5 truncate max-w-[150px]">{{ $area->description }}</span>
                                </div>
                            </div>
                        </td>
                        <td class="py-4 px-6 whitespace-nowrap">
                            <div class="flex flex-col items-start">
                                ...
                                @if($area->estimated_available && $area->status == 'coming_soon')
                                    <span class="text-[10px] text-slate-500 font-medium mt-1 bg-slate-100 px-1.5 py-0.5 rounded">Est: {{ $area->estimated_available->format('d/m/Y') }}</span>
                                @endif
                            </div>
                        </td>
                        <td class="py-4 px-6 text-sm text-slate-600 hidden md:table-cell max-w-xs xl:max-w-md">
                            <p class="truncate" title="{{ $area->description }}">{{ $area->description ?: '-' }}</p>
                        </td>
                        <td class="py-4 px-6 whitespace-nowrap text-center">
                            <span class="text-slate-500 font-medium bg-slate-50 px-2 py-1 rounded-md text-sm border border-slate-100">{{ $area->sort_order ?? '-' }}</span>
                        </td>
                        <td class="py-4 px-6 whitespace-nowrap text-right">
                            <div class="flex items-center justify-end space-x-2">
                                <a href="{{ route('admin.areas.edit', $area->id) }}" class="inline-flex justify-center items-center w-8 h-8 rounded-lg bg-white border border-slate-200 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 hover:border-indigo-200 transition-colors shadow-sm" title="Edit Area">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                                </a>
                                <form action="{{ route('admin.areas.destroy', $area->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus area ini?');">
                                    @csrf
                                    @method('DELETE')
    ...
    @if($areas->count() === 0)
        <div class="text-center py-16 px-4">
            ...
            <a href="{{ route('admin.areas.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-50 border border-indigo-100 rounded-xl font-medium text-sm text-indigo-700 hover:bg-indigo-100 transition-colors">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                Tambah Area Pertama
            </a>
        </div>
