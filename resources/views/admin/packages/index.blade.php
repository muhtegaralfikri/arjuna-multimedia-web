...
<div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
    ...
    <a href="{{ route('admin.packages.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-xl font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-all shadow-sm">
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden">
            ...
            <tbody class="divide-y divide-slate-100" id="sortable-list">
                @foreach($packages as $package)
                    <tr class="hover:bg-slate-50/80 transition-colors group" data-id="{{ $package->id }}">
                        ...
                        <td class="py-4 px-6 whitespace-nowrap">
                            <span class="text-slate-900 font-bold">Rp {{ number_format($package->price_monthly, 0, ',', '.') }}</span>
                        </td>
                        <td class="py-4 px-6 whitespace-nowrap">
                        ...
                        <td class="py-4 px-6 whitespace-nowrap text-right">
                            <div class="flex items-center justify-end space-x-2">
                                <a href="{{ route('admin.packages.edit', $package->id) }}" class="inline-flex justify-center items-center w-8 h-8 rounded-lg bg-white border border-slate-200 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 hover:border-indigo-200 transition-colors shadow-sm" title="Edit Paket">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                                </a>
                                <form action="{{ route('admin.packages.destroy', $package->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus paket ini? Tindakan ini tidak dapat dibatalkan.');">
                                    @csrf
                                    @method('DELETE')
    ...
    @if($packages->count() === 0)
        <div class="text-center py-16 px-4">
            ...
            <a href="{{ route('admin.packages.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-50 border border-indigo-100 rounded-xl font-medium text-sm text-indigo-700 hover:bg-indigo-100 transition-colors">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const saveOrder = async () => {
            ...
            try {
                const response = await fetch('{{ route("admin.packages.reorder") }}', {
                    method: 'POST',
                    headers: {
                ...
                alert('Urutan paket gagal disimpan. Silakan refresh halaman dan coba lagi.');
            }
