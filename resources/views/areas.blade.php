    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-2 gap-4 mb-12 max-w-2xl mx-auto">
            <div class="text-center p-6 bg-success-50 rounded-xl">
                <div class="text-4xl font-bold text-success-600">{{ $availableCount }}</div>
                <div class="text-gray-600">Area Tersedia</div>
            </div>
            <div class="text-center p-6 bg-warning-50 rounded-xl">
                <div class="text-4xl font-bold text-warning-600">{{ $comingSoonCount }}</div>
        ...
        <div id="areas-grid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="area-card bg-white rounded-xl shadow-md overflow-hidden border border-gray-200 hover:shadow-lg transition" data-status="{{ $area->status }}" data-name="{{ strtolower($area->name) }}">
                <div class="p-6">
                    <div class="flex items-start justify-between mb-4">
                        <div>
                            <h3 class="text-xl font-bold text-gray-900">{{ $area->name }}</h3>
                            @if($area->coverage_detail)
                            <p class="text-sm text-gray-500">{{ $area->coverage_detail }}</p>
                            @endif
                        </div>
                    ...
                    @endif
                    @if($area->status === 'coming_soon' && $area->estimated_available)
                    <p class="text-sm text-warning-600">Perkiraan tersedia: {{ $area->estimated_available->format('d M Y') }}</p>
                    @endif
                </div>
            </div>
            @endforeach
